// Migrate an existing DAMSA CSV to RNTuple.
//
...
//
//   ./build/csv2ntuple <in.csv> <out.root> [--limit N]
//
// Schema is picked from the CSV header. Columns absent from the file are left at
// their struct defaults; extra columns in the file are ignored.
...
int main(int argc, char** argv)
{
    ...
    const auto schema = SniffSchema(in);
    if (!schema) { std::fprintf(stderr, "error: unrecognized CSV header in %s\n", in.c_str()); return 1; }

    const auto t0 = std::chrono::steady_clock::now();
    std::size_t n = 0;
    if      (*schema == "alp")      n = Convert<AlpDecayRow>(in, out, limit);
    ...
    std::printf("  %.1f MB -> %.1f MB  (%.1fx smaller)\n",
